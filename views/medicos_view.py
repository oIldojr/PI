...
class TelaCadastroMedico:
    ...
    def _configurar_janela(self):
        self.root.title("Cadastro de Médico")
    ...
    def _cadastrar(self):
        try:
            dados = {
                ...
                'especialidade': self.entries['especialidade'].get()
            }
            
            if any(not valor for valor in dados.values()):
                raise ValueError("Todos os campos são obrigatórios")
                
            medico = self.controller.cadastrar_medico(**dados)
            messagebox.showinfo("Sucesso", f"Médico {medico.nome} cadastrado!")
            self._limpar_campos()  
